...
for (Auth l : env.get_authorities()) {
    for (Lbl lab : env.get_labels()) {
        ...
        idx += "}";
        msk.common_vars[idx] = ops.sample_z();
        mpk.common_vars_g[idx] = ops.lift_g(msk.common_vars[idx]);
    }
}
for (Auth l : env.get_authorities()) {
    for (Lbl lab : env.get_labels()) {
...
for (Attr l_global : env.get_user_attributes()) {
    ...
    usk.non_lone_vars_h[idx] = ops.lift_h(non_lone_randoms[idx]);
}
for (Attr l_global : env.get_user_attributes()) {
    Auth l = env.attr_to_auth(l_global);
    tmp_z = ops.reset_z();
...
for (int j : env.get_lsss_rows()) {
    ...
    idx += "}";
    non_lone_randoms[idx] = ops.sample_z();
    ct.non_lone_vars_g[idx] = ops.lift_g(non_lone_randoms[idx]);
}
for (int j : env.get_lsss_rows()) {
    tmp_z = ops.reset_z();
    acc_z = ops.reset_z();
...
for (int j : env.get_pos_lsss_rows()) {
    ...
    idx += "b'";
    idx += "_{";
    Auth j_local_6 = env.ls_row_to_auth(j);
    idx += env.auth_to_string(j_local_6);
    idx += "}";
    tmp_g = ops.fdh_g(1, idx);
    tmp_g = ops.scale_g(tmp_z, tmp_g);
    acc_g = ops.add_g(acc_g, tmp_g);
...
for (int j : env.get_neg_lsss_rows()) {
    ...
    idx += "b'";
    idx += "_{";
    Auth j_local_3 = env.ls_row_to_auth(j);
    idx += env.auth_to_string(j_local_3);
    idx += "}";
    tmp_g = ops.fdh_g(1, idx);
    tmp_g = ops.scale_g(tmp_z, tmp_g);
    acc_g = ops.add_g(acc_g, tmp_g);
...
for (int j : env.get_positive_linear_combination_idcs()) {
    ...
    idx += env.dedup_idx_to_string(j_local_0);
    idx += "}";
    tmp_g = ct.non_lone_vars_g[idx];
    idx = "";
    idx += "k";
    idx += "_{";
    idx += "2";
    idx += ",";
    Lbl j_local_1 = env.ls_row_to_lbl(j);
    idx += env.lbl_to_string(j_local_1);
    idx += ",";
    Attr j_local_2 = env.ls_row_to_attr(j);
    idx += env.attr_to_string(j_local_2);
    idx += "}";
    tmp_h = usk.polys_h[idx];
    tmp_gt = ops.pair(tmp_g, tmp_h);
    tmp_z = ops.read_z("1");
    aux_z = ops.read_z("-1");
    tmp_z = ops.mul_z(tmp_z, aux_z);
...
for (int j : env.get_negative_linear_combination_idcs()) {
    ...
    idx += env.lbl_to_string(j_local_1);
    idx += ",";
    Attr j_local_2 = env.ls_row_to_alt_attr(j);
    idx += env.attr_to_string(j_local_2);
    idx += "}";
    tmp_h = usk.polys_h[idx];
    tmp_gt = ops.pair(tmp_g, tmp_h);
